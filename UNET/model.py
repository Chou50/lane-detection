...
class ResNetUNet(nn.Module):
    ...
    def forward(self, input):
        ...
        # decoder: unet
        layer4 = self.layer4_11(layer4)
        x = self.upsample(layer4)
        layer3 = self.layer3_11(layer3)

        x = torch.cat([x, layer3], dim=1)
        x = self.conv_up3(x)

        x = self.upsample(x)
        layer2 = self.layer2_11(layer2)
        x = torch.cat([x, layer2], dim=1)
        x = self.conv_up2(x)

        x = self.upsample(x)
        layer1 = self.layer1_11(layer1)
        x = torch.cat([x, layer1], dim=1)
        x = self.conv_up1(x)

        x = self.upsample(x)
        layer0 = self.layer0_11(layer0)
